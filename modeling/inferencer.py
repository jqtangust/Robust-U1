...
from modeling.data.data_utils import pil_img2rgb
from modeling.modeling.bagel.qwen2_navit import NaiveCache
...
class InterleaveInferencer:
    ...
    def gen_image(
        ...
        if learn:
            ...
            if grpo_config is not None:
                sde_kwargs["sample_sde_window_size"] = grpo_config.sample.sde_window_size
                sde_kwargs["sample_sde_window_range"] = grpo_config.sample.sde_window_range
    ...
    def gen_text(
        ...
        if return_ids:
            return unpacked_latent[:, 0].tolist()
        ...
        with torch.autocast(device_type="cuda", enabled=True, dtype=torch.bfloat16):
            ...
            for input_term in input_lists:
                if isinstance(input_term, str):
                    ...
                    # 生成任务用 VAE，理解任务用 ViT
                    gen_context = self.update_context_image(input_term, gen_context, vae=not understanding_output, vit=understanding_output)
            ...
            if understanding_output:
                ...
                if isinstance(img_out, dict):
                    img_out = img_out.get("image", img_out)
