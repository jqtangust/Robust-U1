...
from modeling.data.data_utils import (
    create_sparse_mask, 
...
class Bagel(PreTrainedModel):
    ...
    def generate_image_learn(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=not accelerator.is_local_main_process):
            ...
            if t > cfg_interval[0] and t <= cfg_interval[1]:
                ...
                cfg_text_scale_ = 1.0
                cfg_img_scale_ = 1.0
            # 注意：不使用 accelerator.accumulate()，因为 DeepSpeed ZeRO 不支持 no_sync
            # 而且这里每个 timestep 都单独做 optimizer.step()，不需要梯度累积
            v_t = self._forward_flow(
                x_t=latents[i],
                timestep=timestep, 
                packed_vae_token_indexes=packed_vae_token_indexes,
    ...
    def generate_text(
        ...
        while step < max_length:
            ...
            # After <|im_end|>, predict one more token (image / text / end action).
